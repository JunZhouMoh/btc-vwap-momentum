...
def _normalize_b64(value: Optional[str]) -> str:
    text = _clean_str(value)
    if not text:
        return ""

    # py_clob_client decodes API secret as base64; missing trailing '=' from
    # copy/paste or env quoting often causes "Incorrect padding".
    missing = len(text) % 4
    if missing:
        text += "=" * (4 - missing)
    return text
...
def _is_auth_error(message: str) -> bool:
    ...
    return (
        ...
        or "unauthorized" in text and "api key" in text
    )
...
class OrderExecutor:
    ...
    def __init__(
        ...
        self.private_key = _clean_str(private_key)
        self.api_key = _clean_str(api_key)
        self.api_secret = _normalize_b64(api_secret)
        self.api_passphrase = _clean_str(api_passphrase)
        self.clob_host = _clean_str(clob_host) or "https://clob.polymarket.com"
        self.chain_id = _coerce_int(chain_id, 137)
        self.signature_type = _coerce_int(signature_type, 0)
        self.funder_address = _clean_str(funder_address) or None
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            order_logger.info(f"  API Key Length: {len(self.api_key)}")
            order_logger.info(f"  API Secret Length: {len(self.api_secret)}")
            order_logger.info(f"  API Passphrase Length: {len(self.api_passphrase)}")
    ...
    async def place_fak_order(
        ...
        try:
            # py_clob_client_v2 can create and post in one call.
    ...
    async def execute_entry(
        ...
        while contracts_bought < contracts_needed and attempt < config.max_retries:
            ...
            if not success:
                ...
                # Invalid/mismatched API creds are deterministic; retries will not help.
                if _is_auth_error(error_msg):
                    ...
                    order_logger.info("=" * 60)
                    order_logger.info("ENTRY EXECUTION COMPLETE (AUTH FAILURE)")
                    order_logger.info("  Success: False")
