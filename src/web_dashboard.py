    ...
    function tick(){
      ...
      r.onreadystatechange=function(){
        ...
        try{
          ...
          var nextWinResult=d.next_window_order_result||null; if(nextWinResult){ var nwrStatus=String(nextWinResult.status||'?'); var nwrDir=String(nextWinResult.direction||'?'); var nwrAmt=(nextWinResult.amount_usd!=null)?numFmt(nextWinResult.amount_usd,2):'\u2014'; var nwrLine=''; if(nwrStatus==='success'){ var nwrContracts=(nextWinResult.contracts!=null)?numFmt(nextWinResult.contracts,2):'\u2014'; var nwrPrice=(nextWinResult.price!=null)?numFmt(nextWinResult.price,4):'\u2014'; nwrLine='\u2705 Sent: BUY '+esc(nwrDir)+' $'+esc(nwrAmt)+' | '+esc(nwrContracts)+' contracts @ '+esc(nwrPrice); var nwrUnreal='\u2014'; if(nextWinResult.contracts!=null&&nextWinResult.price!=null){ var nextTokenData=(nwrDir==='UP')?d.next_up:d.next_down; var nextCurrentPrice=(nextTokenData&&nextTokenData.book&&nextTokenData.book.last_price)?nextTokenData.book.last_price:null; if(nextCurrentPrice!=null){ var nwrUnrealPnL=(nextCurrentPrice-nextWinResult.price)*nextWinResult.contracts; nwrUnreal=numFmtSigned(nwrUnrealPnL,2); } } nwrLine+='<br/>Unreal: $'+nwrUnreal; }else{ var nwrErr=String(nextWinResult.error||'Unknown'); nwrLine='\u274c Failed: BUY '+esc(nwrDir)+' $'+esc(nwrAmt)+' - '+esc(nwrErr); } tHtml+='<br/>Next Window Order Result: '+nwrLine+'<br/>'; }
