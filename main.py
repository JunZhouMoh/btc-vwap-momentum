...
order_handler = logging.FileHandler(str(LOG_DIR / "orders.log"))
...
hedge_handler = logging.FileHandler(str(LOG_DIR / "hedges.log"))
...
signal_handler = logging.FileHandler(str(LOG_DIR / "signals.log"))
...
class TradingStats:
    def __init__(self, log_file: str = "logs/trading_log.json"):
        self.log_file = resolve_runtime_path(log_file)
...
class Dashboard:
    def __init__(self, state: MarketState, stats: TradingStats, config: Any):
        ...
        self._signals_csv_path = resolve_runtime_path("logs/signals.csv")
        self._bot_log_path = resolve_runtime_path("logs/bot.log")
        self._latest5_csv_path = resolve_runtime_path("logs/latest_5_windows.csv")
    ...
    def _write_latest_5_windows_from_history(self, periods: int = 5) -> None:
        ...
        try:
            trading_log = resolve_runtime_path("logs/trading_log.json")
    ...
    def _get_btc_buffer_status(self) -> Optional[Dict[str, float]]:
        ...
        return {
            ...
            "avg_abs_usd": buffer_stats["avg_abs_usd"],
            "decayed_abs_usd": buffer_stats["decayed_abs_usd"],
            "avg_abs_pct": buffer_stats["avg_abs_pct"],
            "decayed_abs_pct": buffer_stats["decayed_abs_pct"],
    ...
    def create_strategy_panel(self) -> Panel:
        ...
        if btc_buffer:
            metric = btc_buffer.get("buffer_metric", "average")
            lines.insert(
                7,
                f"BTC Buffer:  ${btc_buffer['current_abs_usd']:,.2f} vs ${btc_buffer['buffer_abs_usd']:,.2f} "
                f"({'OK' if btc_buffer_ok else 'WAIT'}) | avg ${btc_buffer['avg_abs_usd']:,.2f}, "
                f"decay ${btc_buffer['decayed_abs_usd']:,.2f}, use {metric}"
            )
...
class LiveTradingBot:
    def __init__(self):
        ...
        self.stats = TradingStats(str(resolve_runtime_path("logs/trading_log.json")))
